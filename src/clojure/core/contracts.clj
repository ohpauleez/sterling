...
;; Dispatch on the `supers` list of the thing we're trying to contrain
(defmulti constrain-on-supers
  (fn [x kontracts]
    (take 1 (filter (supers (if (class? x) x (class x)))
                    (keys (methods constrain-on-supers))))))

;; A generic contraint for all record types - akin to Trammel's defcontrainedrecord
(defmethod constrain-on-supers clojure.lang.IRecord [t kontracts]
  "GOT RECORD")

(defprotocol Constrain
  "This is the default mechanism to contrain something (a function, record, var)
  with contracts"
  (-with-constraints [t] [t kontract] [t kontract & more-kontracts]
                    "The single, fundamental function to constrain something"))

;;TODO this needs to be organized better, the & more is problematic
(defn with-constraints
  ([t] t)
  ([t c] (-with-constraints t c)))

(extend-protocol Constrain
  clojure.lang.Fn
  (-with-constraints
    ([f] f)
    ([f c] (partial c f))
    ([f c & more] (apply -with-constraints (-with-constaints f c) more)))

  ;;Fall through to a multimethod that dispatches on satisfies? - maybe -
  ;;so we can generically constrain things like Records (ala IRecord)
  java.lang.Object
  (-with-constraints
    ([t] t)
    ([t c] (constrain-on-supers t [c]))
    ([t c & more] (constrain-on-supers t (conj more c)))))

(defn with-constraints-fn
  "DEPRECATED: A contract combinator.
