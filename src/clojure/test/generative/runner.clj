              ...
              (repeatedly
               (fn []
                 (into [] (map #(%) (:clojure.test.generative/arg-fns (meta v)
                                                                      (-> (var-get v)
                                                                        :clojure.core.specs/args
                                                                        tagger/extract-arg-fns)))))))}])

(defmethod var-tests nil [v] (println "I got nothing"))
...
(defn run-example
  "Like run-iter, but takes a test-fn var"
  [test-var]
  (let [[test-map] (var-tests (if (var? test-var)
                                test-var (throw (Exception. "run-example can only be passed in a var"))))
        args (first ((:inputs test-map)))
        res (apply (:fn test-map) args)]
    (assoc test-map :result res
                    :example `(~(:name test-map) ~@args))))


(defn run-generative-tests
  "Run generative tests."
  ([]
   (run-generative-tests [*ns*] (-> (Runtime/getRuntime) .availableProcessors) 1000))
  ([msec]
   (run-generative-tests [*ns*] (-> (Runtime/getRuntime) .availableProcessors) msec))
  ([nthreads msec]
   (run-generative-tests [*ns*] nthreads msec))
  ([nses nthreads msec]
  (let [c (count (->> (apply find-vars-in-namespaces nses)
                      (mapcat tests)))]
    (println "Running tests for the following namespaces:" nses
             "\nAcross nthreads:" nthreads
             "\nExtracting tests.  Found..." c)
    (when-not (zero? c)
